...
p(note). @sync()@ doesn't re-fetch the list as it was already fetched.

p(note). @forceSync()@ forces the re-fetch of the list and all elements are synchronized with the datastore. Like that, you control exactly how you want to fetch data from the datastore after the first synchronization.
...
p(note). @addAll(Collection)@ is a convenient function to add several elements to a list without putting them in a Collection.

p(note). once you have a query with @asQuery()@, you can do what you want with it as it is a query ;)
