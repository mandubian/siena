...
bq. *IMPORTANT TO KNOW*: Siena is aimed at bridging SQL/NoSQL. *Almost all NoSQL DB have a limitation compared to SQL DB which is the absence of JOIN request*. In NoSQL, when you fetch the originating model, you can't fetch the targeted model in the same query (which is possible in a SQL DB with a left join). 
